...
class Dataset_Pretrain(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        data_dict = {
            ...
            'NSG2020': Dataset_Grid,
            'NSG2023': Dataset_Grid,
        ...
        for file in ['ETTh1', 'ETTh2', 'ETTm1', 'ETTm2']:
            Data = data_dict[file]('dataset/ETT/', flag, size, features, file + '.csv', target, scale, timeenc,
        ...
        for file in ['electricity']:
            ...
            print('dataset {0}, length {1}'.format(file, self.num_channels[-1] * len(Data)))
            self.bounds.append(self.bounds[-1] + self.num_channels[-1] * len(Data))
        # for file in ['NSG2020', 'NSG2023']:
        #     target = [f'a{i}' for i in range(5)]
        #     Data = data_dict[file]('dataset/' + file + '/', flag, size, features, file + '.csv', target, scale, timeenc,
        #                            freq, percent=percent)
        #     self.num_channels.append(Data.data_x.shape[-1])
        #     self.data_xs.append(Data.data_x)
...
class Dataset_Grid(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                features='M', data_path='power_grid.csv',
                target='All', scale=True, timeenc=0, freq='h', seasonal_patterns=None, percent=100):
    # size [seq_len, label_len, pred_len]
    # info
        if size == None:
            self.seq_len = 24 * 4 * 4
        ...
        self.features = features
        # self.target = target
        self.target = [f'a{i}' for i in range(5)]
        self.scale = scale
        self.timeenc = timeenc
        self.freq = freq
    ...
    def __read_data__(self):
        ...
        cols = list(df_raw.columns)
        # cols.remove(self.target)
        cols = [col for col in cols if col not in self.target]
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + self.target]
        num_train = int(len(df_raw) * 0.7)
        num_test = int(len(df_raw) * 0.2)
